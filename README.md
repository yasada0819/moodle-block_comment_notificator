...
このプラグインは、既存の `local_db_com_notificator` を元に改訂し、コース単位で通知対象を制御できるブロック版として作られています。Moodle 全体へ一律に適用するのではなく、ブロックを設置したコースごとに通知対象を調整できます。
...
This plugin was revised from the existing `local_db_com_notificator` plugin and rebuilt as a block version that can control notification targets at the course level. Instead of applying the same notification behavior across the whole Moodle site, it lets each course control which activities should send notifications.
